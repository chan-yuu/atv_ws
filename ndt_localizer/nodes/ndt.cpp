...
NdtLocalizer::NdtLocalizer(ros::NodeHandle &nh, ros::NodeHandle &private_nh):nh_(nh), private_nh_(private_nh), tf2_listener_(tf2_buffer_){
  ...
  map_pub = nh_.advertise<car_interfaces::GpsImuInterface>("map_pose", 10);
  ...
  const sensor_msgs::PointCloud2::ConstPtr & sensor_points_sensorTF_msg_ptr)
{
  ...
  if (
    ...
    ROS_INFO_STREAM("\033[1;31m Not Converged !!! : ");// 红
    // ROS_INFO_STREAM("\033[1;32m init reference success !!! : ");// 绿
  
  } else {
    skipping_publish_num = 0;
    ROS_INFO_STREAM("\033[1;32m init localization success !!! : ");// 绿
  }
  ...
  Eigen::Matrix3f map_rotation_matrix = result_pose_matrix.block<3, 3>(0, 0);
  Eigen::Vector3f map_euler = map_rotation_matrix.eulerAngles(2,1,0);
  ...
  map_msg.roll = map_euler(0)*180/M_PI;
  map_msg.yaw = map_euler(0)*180/M_PI;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber init_sub = nh.subscribe<std_msgs::Int32>("/run_hybrid_astar/plan_flag", 10, initCallback);
