  ...
  const sensor_msgs::PointCloud2::ConstPtr & sensor_points_sensorTF_msg_ptr)
{
  ...
  // map_msg.yaw = map_euler(0);
   map_msg.pitch = 0;
   map_msg.roll = 0;
  // map_msg.posX=map_translation(0);
  // map_msg.posY=map_translation(1);
 
  if(speedZero)
  {
     map_msg.yaw = lastYaw*180/M_PI;
  }
  else
  {
    if((map_msg.x-lastX)*(map_msg.x-lastX)+(map_msg.y-lastY)*(map_msg.y-lastY)>0.09)
    {
     double yaw = calcHeading(lastX,lastY,map_msg.x,map_msg.y);
