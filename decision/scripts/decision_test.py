...
from car_interfaces.msg import PathSpeedCtrlInterface,CameraInterface, GpsImuInterface, GlobalPathPlanningInterface, DecisionInterface
...
class DecisionNode:
    def __init__(self):
        ...
        self.obu_up_msg = {}
        self.lane_change_request_to_decision_msg = {}
    ...
    def camera_Callback_category(self, msg):
        # perceive_up_msg = json.loads(msg.data)
        print("category")
        # self.perceive_up_msg['tag'] = perceive_up_msg['tag']
        # self.perceive_up_msg['detail'] = perceive_up_msg['detail']
    ...
    def calculate_distance(self,curp, now_p):
        return math.sqrt(math.pow(curp[0]-now_p[0], 2)+math.pow(curp[1]-now_p[1], 2))
    ...
    def main(self):
        ...
        rospy.Subscriber("/decision_data_from_camera_category", CameraInterface, self.camera_Callback_category)      

        rospy.Subscriber("/plan_to_decision", CameraInterface, self.plan_Callback_category)      
        ...
        #     if self.is_exit:
        #         break
        #     if self.gps_flag and self.plan_flag and self.control_flag:
        #         break
        #     rospy.sleep(0.1)
        #     print(11)

        while not rospy.is_shutdown():
            # print(self.control_flag,self.gps_flag,self.plan_flag)
            if self.gps_flag and self.plan_flag:
                start_time = time.time()
                # x_list_global = []
                ...
                if self.local_Map_dict and self.lidar_know_flag:
                    ...
                    # 紧急避障：
                    if self.front_obj is not None and 0 < self.front_obj['s'] < 5:
                        Emergency_brake = True
                    else:
                        Emergency_brake = False

                    # 换道：
                    if self.front_obj is not None and 0 < self.front_obj['s'] < 15.5 and self.laneid == 0:
                        Emergency_brake_dict = {"Emergency_brake_decision": True}
                        # self.Change_Line = 1
                        # self.laneid = 1
                        # dis_lane = c
                    ...
                    print(flag_true,self.obj_list)
                    if flag_true and self.laneid == 0:
                        curPos_now = [self.ego_content['UTM_x'], self.ego_content['UTM_y']]
                        while True:
                            newPos = [self.ego_content['UTM_x'], self.ego_content['UTM_y']]
                            self.dis_change = self.calculate_distance(curPos_now, newPos)
                            print("dis_change", self.dis_change)
                            if self.dis_change < 15:
                                ...
                                msg_send_to_vcu.dspeed = self.dspeed
                    ...
                    if flag_true and self.laneid == 1:
                        msg_send_to_vcu.Brake_En = self.Brake_En
                        msg_send_to_vcu.Target_Travel = self.Target_Travel
                    else:
                        msg_send_to_vcu.Brake_En = 0
                        msg_send_to_vcu.Target_Travel = 0
                    ...
                    # 浓雾天气
                    if self.is_fog == 1 and self.laneid == 0:
                        # self.dspeed = -2
                        # self.Change_Line = 2
                        # msg_send_to_vcu.Brake_En = self.Brake_En
                        # msg_send_to_vcu.Target_Travel = self.Target_Travel
                    ...
                    msg_send_to_vcu.dspeed = self.dspeed # 网联减速
                    if Emergency_brake:
                        msg_send_to_vcu.Brake_En = 1
                        msg_send_to_vcu.Target_Travel = 20
                    else:
                        msg_send_to_vcu.Brake_En = 0
                        msg_send_to_vcu.Target_Travel = 0
                    ...
                    if self.Change_Line == 1:
                        msg_send_to_vcu.light = 1
                        
                    elif self.Change_Line == 2:
                        msg_send_to_vcu.light = 2
                    else:
                        msg_send_to_vcu.light = 0
